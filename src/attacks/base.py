...
class Attack(object):
    ...
    def run_defense_training(self) -> None:
        """
        Train on the full dataset (no train/val split). Intended for adversarial
        defenses that need a surrogate trained on every trace (``run_attack.py -d``).
        """
        self.logger.info(
            "Defense training mode (-d): %d traces, %d epochs, no CV holdout",
            len(self.flist),
            self.args.epochs,
        )
        _, train_loader = self._get_data(self.flist, self.labels, self.extract, is_train=True)
        ...
        trainer = create_supervised_trainer(
            model, optimizer, criterion, self.device, amp_mode=amp_mode
        )

        @trainer.on(Events.EPOCH_COMPLETED)
        def log_training_loss(engine: Engine):
            self.logger.info(
                "Defense train epoch %d/%d | loss: %.4f",
                engine.state.epoch,
                self.args.epochs,
                engine.state.output,
            )
        ...
        checkpoints_dir = os.path.normpath(self.args.checkpoints.rstrip(os.sep))
        save_defense_final_model(
            model,
            checkpoints_dir,
            attack_type_name,
